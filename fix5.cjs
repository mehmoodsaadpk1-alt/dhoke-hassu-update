...
const regex = /<img src=\{composerImagePreview\} alt="Selected upload" className="w-full h-auto object-contain block" \/>[\s\S]*?const renderPost = \(post: any\) => \{/;

const replacement = `<img src={composerImagePreview} alt="Selected upload" className="w-full h-auto object-contain block" />
            <button 
              onClick={() => {
...
console.log('Fixed AppShell.tsx robustly');
