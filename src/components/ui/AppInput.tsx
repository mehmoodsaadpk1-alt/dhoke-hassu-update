    ...
    <div className="w-full space-y-1.5">
      ...
      <div className="relative">
        ...
        <input
          ...
          disabled={disabled}
          dir="auto"
          className={`w-full bg-slate-50 border rounded-2xl text-sm placeholder-slate-400 transition-all duration-200 disabled:opacity-50 disabled:bg-slate-100/60 text-start bidi-isolate
            ${leadingIcon ? 'ps-10' : 'ps-4'}
            ${trailingIcon || isPassword ? 'pe-10' : 'pe-4'}
            ${error ? 'border-error focus:border-error focus:ring-red-100' : 'border-slate-200 focus:border-primary focus:bg-white focus:ring-2 focus:ring-emerald-100'}
            py-2.5 h-10.5
            ${className}`}
          {...props}
        />
  ...
  return (
    <div className="w-full space-y-1.5">
      ...
      <textarea
        disabled={disabled}
        dir="auto"
        className={`w-full bg-slate-50 border rounded-2xl text-sm placeholder-slate-400 transition-all duration-200 resize-none disabled:opacity-50 disabled:bg-slate-100/60 px-4 py-2.5 min-h-[96px] text-start bidi-isolate
          ${error ? 'border-error focus:border-error focus:ring-red-100' : 'border-slate-200 focus:border-primary focus:bg-white focus:ring-2 focus:ring-emerald-100'}
          ${className}`}
        {...props}
      />
  ...
  return (
    <div className="w-full space-y-1.5">
      ...
      <div className="relative">
        ...
        <select
          disabled={disabled}
          className={`w-full bg-slate-50 border rounded-2xl text-sm transition-all duration-200 appearance-none cursor-pointer disabled:opacity-50 disabled:bg-slate-100/60 text-start
            ${leadingIcon ? 'ps-10' : 'ps-4'}
            pe-10 py-2.5 h-10.5
            ${error ? 'border-error focus:border-error focus:ring-red-100' : 'border-slate-200 focus:border-primary focus:bg-white focus:ring-2 focus:ring-emerald-100'}
            ${className}`}
