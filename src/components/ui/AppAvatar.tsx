...
export const AppAvatar: React.FC<AppAvatarProps> = ({
  ...
  const safeName = typeof name === 'string' && name?.trim().length > 0 ? name?.trim() : 'Unknown User';
  ...
  const handleClick = (e: React.MouseEvent) => {
    ...
    if (clickable) {
      ...
      if (openUser) {
        openUser(safeName, avatar);
      } else {
        const url = `/profile/view?name=${encodeURIComponent(safeName)}${avatar ? `&avatar=${encodeURIComponent(avatar)}` : ''}`;
