      ...
      <div className="bg-white p-4 rounded-3xl border border-slate-200 shadow-sm flex flex-col sm:flex-row gap-4 items-center">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full ps-9 pe-4 py-2 bg-slate-50 border border-slate-200 rounded-2xl text-xs font-semibold focus:outline-none focus:ring-2 focus:ring-emerald-500/20 focus:border-emerald-500 transition-all"
          />
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full sm:w-auto">
          ...
          <label className="flex items-center gap-2 cursor-pointer bg-red-50 text-red-700 px-3 py-2 rounded-2xl border border-red-100">
            <input 
              type="checkbox"
                    ...
                    <tr key={post.id} className={`hover:bg-slate-50/50 ${status === 'deleted' ? 'opacity-60' : ''}`}>
                        ...
                        <div className="flex items-start gap-2">
                          {hasMedia && (
                            <div className="flex-shrink-0 mt-0.5">
                              {post.videoUrl ? <VideoIcon className="w-3.5 h-3.5 text-emerald-500" /> : <ImageIcon className="w-3.5 h-3.5 text-green-500" />}
                            </div>
                          )}
                      ...
                      <td className="py-3 px-6">
                        <div className="flex flex-col items-start gap-1">
                          <span className={`px-2 py-0.5 rounded text-[9px] font-black uppercase ${
                            ...
                            status === 'hidden' ? 'bg-emerald-50 text-amber-700' :
                            'bg-red-50 text-red-700'
                          }`}>
                            {status}
                          </span>
                          {post.pinned && (
                            <span className="px-2 py-0.5 rounded text-[9px] font-black uppercase bg-emerald-50 text-emerald-700 flex items-center gap-1">
                      ...
                      <td className="py-3 px-6 text-end space-x-1 whitespace-nowrap">
                        ...
                        <button
                          ...
                          className={`p-1.5 rounded-xl transition-colors border-none bg-transparent cursor-pointer ${post.pinned ? 'text-emerald-600 bg-emerald-50' : 'text-slate-400 hover:bg-slate-50'}`}
                        ...
                        {status === 'active' ? (
                          ...
                          <button
                            ...
                            className="p-1.5 text-green-600 hover:bg-green-50 rounded-xl transition-colors border-none bg-transparent cursor-pointer"
                            title="Unhide Post"
                          >
                            <Eye className="w-4 h-4" />
                          </button>
                        ...
                        {status !== 'deleted' && (
                          <button
                            ...
                            className="p-1.5 text-red-500 hover:bg-red-50 rounded-xl transition-colors border-none bg-transparent cursor-pointer"
                            title="Delete Post"
                          >
            ...
            <div className="flex gap-1">
              {Array.from({ length: totalPages }).map((_, idx) => (
                <button
                  ...
                  className={`w-7 h-7 rounded-xl text-xs font-bold transition-colors border-none cursor-pointer ${
                    currentPage === idx + 1 
