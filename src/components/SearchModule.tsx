...
const pollsBannerMap = useAdRotator('Polls & Opinions', 1, 1, 'Banner');
  ...
  useEffect(() => {
    ...
    const fetchUsers = async () => {
      try {
        console.log("[TRACE] 1. fetchUsers called with query:", query.trim());
        const users = await dbSearchUsers(query.trim());
        setRealUsers(users || []);
      } catch (err) {
  ...
  const allUsers = [...realUsers, currentUser].filter(Boolean);
  ...
  const matchesSearch = (item: UniformSearchResult, searchStr: string): boolean => {
    ...
    // 1. Users fetched from DB are ALREADY filtered by dbSearchUsers.
    // Duplicate client-side filtering causes mismatches and hides valid DB results.
    // If it's a user, we trust the DB's search result unconditionally.
    if (item.category === 'users' || item.rawType === 'user') {
      return true;
    }
  ...
  // Add TRACE logs for debugging search flow
  useEffect(() => {
    if (query?.trim()) {
      console.log("[TRACE] 6. Final filtered array in SearchModule:", filteredResults);
      console.log("[TRACE] 7. Rendered array length:", filteredResults.length);
    }
  ...
  const typingSuggestions = query?.trim() 
    ? rawResults
        ...
        .map(item => {
          ...
          if (item.rawData?.company && item.rawData.company?.toLowerCase().includes(lowerQuery)) {
            return item.rawData.company;
          }
          if (item.category === 'users') {
            if (item.rawData?.username && item.rawData.username.toLowerCase().includes(lowerQuery)) {
              return item.rawData.username;
            }
            const mobile = String(item.rawData?.mobileNumber || item.rawData?.contactNumber || '').replace(/[-\s()+]/g, '').toLowerCase();
            if (mobile && mobile.includes(cleanQuery)) {
              return item.rawData.mobileNumber || item.rawData.contactNumber;
            }
          }
          return item.title;
        })
  ...
  return (
    <div className="w-full max-w-4xl mx-auto px-4 sm:px-6 py-4 space-y-6 animate-fade-in" id="global-search-container">
      ...
      {currentPath.startsWith('/search/results') && (
