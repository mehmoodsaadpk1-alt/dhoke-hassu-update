...
export async function dbSaveUserProfile(profile: User): Promise<boolean> {
    ...
    try {
      ...
      try {
        const { data, error } = await supabase.from('profiles').select('*').eq('user_id', profile.id).single();
        if (!error && data) {
          existingProfile = data;
          console.log("[dbSaveUserProfile] Existing profile loaded for merging.");
        } else if (error && error.code === 'PGRST116') {
          console.log("[dbSaveUserProfile] No existing profile found. Creation allowed.");
        } else if (error) {
          console.error("[dbSaveUserProfile] Aborting save: Error fetching existing profile for merge:", error);
          throw new Error("Aborting profile save to prevent data loss due to read failure.");
        }
      } catch (err) {
        console.error("[dbSaveUserProfile] Aborting save: Exception fetching existing profile:", err);
        throw err;
      }

      const existingSocialLinks = typeof existingProfile?.socialLinks === 'string' ? JSON.parse(existingProfile.socialLinks) : (existingProfile?.socialLinks || {});
      const newSocialLinks = typeof profile.socialLinks === 'string' ? JSON.parse(profile.socialLinks) : (profile.socialLinks || {});

      const mergedSocialLinks = {
        ...existingSocialLinks,
        ...newSocialLinks
      };
      delete mergedSocialLinks.coverPhoto;

      const payload: Record<string, any> = {
        user_id: profile.id,
        full_name: profile.fullName || existingProfile?.full_name,
        email: profile.email || existingProfile?.email,
        profile_photo: profile.profilePhoto !== undefined ? profile.profilePhoto : existingProfile?.profile_photo,
        mobileNumber: profile.mobileNumber !== undefined ? profile.mobileNumber : existingProfile?.mobileNumber,
        username: profile.username !== undefined ? profile.username : existingProfile?.username,
        bio: profile.bio !== undefined ? profile.bio : existingProfile?.bio,
        joinDate: profile.joinDate !== undefined ? profile.joinDate : existingProfile?.joinDate,
        reputationScore: profile.reputationScore ?? existingProfile?.reputationScore ?? 100,
        verified: profile.verified ?? existingProfile?.verified ?? false,
        coverPhoto: profile.coverPhoto !== undefined ? profile.coverPhoto : existingProfile?.coverPhoto,
        contactNumber: profile.contactNumber !== undefined ? profile.contactNumber : existingProfile?.contactNumber,
        socialLinks: {
          ...mergedSocialLinks,
          gender: profile.gender !== undefined ? profile.gender : (existingProfile?.gender || existingSocialLinks.gender || null),
          dateOfBirth: profile.dateOfBirth !== undefined ? profile.dateOfBirth : (existingProfile?.date_of_birth || existingSocialLinks.dateOfBirth || null),
          provinceId: profile.provinceId !== undefined ? profile.provinceId : (existingProfile?.province_id || existingSocialLinks.provinceId || null),
          cityId: profile.cityId !== undefined ? profile.cityId : (existingProfile?.city_id || existingSocialLinks.cityId || null),
          areaId: profile.areaId !== undefined ? profile.areaId : (existingProfile?.area_id || existingSocialLinks.areaId || null),
          latitude: profile.latitude !== undefined ? profile.latitude : (existingProfile?.latitude || existingSocialLinks.latitude || null),
          longitude: profile.longitude !== undefined ? profile.longitude : (existingProfile?.longitude || existingSocialLinks.longitude || null)
        },
        badges: profile.badges && profile.badges.length > 0 ? profile.badges : (existingProfile?.badges || [])
      };
...
export async function dbGetStories(currentUserId: string, fallback: Story[] = []): Promise<Story[]> {
  return safeDbCall(async () => {
    ...
    // We do NOT join with profiles directly via select because stories doesn't have a foreign key to profiles
    const { data, error } = await supabase!
      .from('stories')
      .select('*')
    ...
    const userIds = [...new Set((data || []).map(s => (s.custom_audience_ids && s.custom_audience_ids.length > 0) ? s.custom_audience_ids[0] : s.author).filter(Boolean))];
    const profilesMap: Record<string, any> = {};
    if (userIds.length > 0) {
      const { data: profilesData } = await supabase!.from('profiles').select('user_id, full_name, profile_photo, area').in('user_id', userIds);
    ...
    const stories = (data || []).map((s: any) => {
      const userId = (s.custom_audience_ids && s.custom_audience_ids.length > 0) ? s.custom_audience_ids[0] : (s.user_id || s.author);
...
// Global cache to prevent multiple updates per session due to React re-renders
const trackedStoryAdEvents = new Set<string>();
...
export async function dbLogStoryAdAnalytics(adId: string, action: 'impression' | 'click' | 'completion' | 'skip' | 'exit'): Promise<void> {
  try {
    ...
    const cacheKey = `${adId}-${action}`;
    if (trackedStoryAdEvents.has(cacheKey)) return;
    trackedStoryAdEvents.add(cacheKey);

    let rpcName = '';
    
    if (action === 'impression') {
      console.log('Story Ad Opened:');
      console.log(`Ad ID: ${adId}`);
      console.log('Calling incrementStoryAdView():');
      rpcName = 'increment_story_ad_view';
    } else if (action === 'click') {
      console.log('Story Ad Clicked:');
      console.log(`Ad ID: ${adId}`);
      console.log('Calling incrementStoryAdClick():');
      rpcName = 'increment_story_ad_click';
    } else if (action === 'completion') {
      console.log('Story Ad Completed:');
      console.log(`Ad ID: ${adId}`);
      console.log('Calling incrementStoryAdCompletion():');
      rpcName = 'increment_story_ad_completion';
    } else {
      // Skips and Exits fallbacks (RPC created but user only asked for three main ones)
      return; 
    }

    const { error: updateError } = await supabase!.rpc(rpcName, { ad_id: adId });

    if (updateError) {
      console.log('Database Update Result: FAILED', updateError);
      throw updateError;
    } else {
...
export async function dbGetPosts(fallback: Post[], groupId?: string): Promise<Post[]> {
  ...
  try {
    ...
    const shareIdsByType: Record<string, string[]> = {};
    data.forEach((p: any) => {
      let stype = null;
      let sid = null;
      if (typeof p.text_content === 'string' && p.text_content.startsWith('[SHARE_DATA]')) {
        try {
          const parsed = JSON.parse(p.text_content.substring('[SHARE_DATA]'.length));
          p.post_type = 'share';
          stype = parsed.entityType;
          sid = parsed.entityId;
          p.shared_caption = parsed.caption;
          p.shared_entity_type = stype;
          p.shared_entity_id = sid;
        } catch(e){}
      }
      if (p.post_type === 'share' && p.shared_entity_type && p.shared_entity_id) {
        if (!shareIdsByType[p.shared_entity_type]) shareIdsByType[p.shared_entity_type] = [];
        shareIdsByType[p.shared_entity_type].push(p.shared_entity_id);
      }
    });

    const sharedEntitiesData: Record<string, any> = {};
    
    await Promise.all(Object.entries(shareIdsByType).map(async ([type, ids]) => {
      if (!ids.length) return;
      const tableName = getTableNameFromEntityType(type);
      if (!tableName) return;
      ...
      try {
        ...
        if (entityData) {
          entityData.forEach(e => {
            ...
            // Convert stored video_path to a public URL if needed
            let vUrl = e.video_url || e.videoUrl || '';
            if (vUrl && !vUrl.startsWith('http')) {
              const publicUrl = supabase!.storage.from('posts').getPublicUrl(vUrl).data.publicUrl;
              vUrl = publicUrl;
            }
            e.videoUrl = vUrl;
            e.video_url = vUrl;
    ...
    return data.map((p: any) => {
      ...
      const sharedEntity = p.post_type === 'share' && p.shared_entity_id ? sharedEntitiesData[`${p.shared_entity_type}_${p.shared_entity_id}`] : undefined;
      
      let sharedOriginalEntity = undefined;
      if (sharedEntity) {
        let title, subtitle, content, image, videoUrl;
        const type = p.shared_entity_type;
        ...
        if (type === 'post' || type === 'poll' || type === 'alert' || type === 'share' || type === 'general') {
          ...
          title = entity.title || entity.name || 'Shared Item';
          content = entity.description || entity.text || '';
          image = entity.image_url || entity.image || '';
        }
        
        sharedOriginalEntity = {
          ...entity,
          title, subtitle, content, image, videoUrl
        };
      }

      let videoUrl = p.video_url || p.videoUrl || '';
      if (videoUrl && !videoUrl.startsWith('http')) {
        videoUrl = supabase!.storage.from('posts').getPublicUrl(videoUrl).data.publicUrl;
      }

      const author = profile?.full_name ?? p.author ?? 'Local Resident';
      const avatar = profile?.profile_photo ?? p.avatar ?? 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=120';
      ...
      const parsedPost = {
        ...
        sharedEntityType: p.shared_entity_type || undefined,
        sharedEntityId: p.shared_entity_id || undefined,
        sharedCaption: p.shared_caption || undefined,
        sharedOriginalEntity: sharedOriginalEntity,
        lfImages: lfImages,
        images: lfImages,
...
export async function dbUpdatePostStatus(postId: string, status: 'active' | 'hidden' | 'deleted'): Promise<boolean> {
  if (!isSupabaseConfigured || !supabase) return false;
  try {
    const { error } = await supabase.from('posts').update({ status }).eq('id', postId);
    if (error) {
      console.warn("Failed to update post status in Supabase:", error);
      return false;
    }
    return true;
  } catch (err) {
    console.warn("Exception in dbUpdatePostStatus:", err);
    return false;
  }
}

export async function dbTogglePostPin(postId: string, pinned: boolean): Promise<boolean> {
  if (!isSupabaseConfigured || !supabase) return false;
  try {
    const { error } = await supabase.from('posts').update({ pinned }).eq('id', postId);
    if (error) {
      console.warn("Failed to toggle post pin in Supabase:", error);
      return false;
    }
    return true;
  } catch (err) {
    console.warn("Exception in dbTogglePostPin:", err);
    return false;
  }
}
...
export function matchesUserQuery(userObj: any, searchQuery: string): boolean {
  if (!searchQuery || !userObj) return false;
  
  const lower = String(searchQuery).toLowerCase();
  const cleanSearch = String(searchQuery).replace(/[-\s()+]/g, '').toLowerCase();

  const fullName = String(userObj.fullName || userObj.full_name || userObj.name || '').toLowerCase();
  const username = String(userObj.username || '').toLowerCase();
  const displayName = String(userObj.displayName || '').toLowerCase();
  
  const mobile = String(userObj.mobileNumber || userObj.contact || '').replace(/[-\s()+]/g, '').toLowerCase();
  const contact = String(userObj.contactNumber || '').replace(/[-\s()+]/g, '').toLowerCase();

  return (
    (fullName && fullName.includes(lower)) ||
    (username && username.includes(lower)) ||
    (displayName && displayName.includes(lower)) ||
    (mobile && mobile.includes(cleanSearch)) ||
    (contact && contact.includes(cleanSearch))
  );
}
...
export async function dbSearchUsers(searchQuery: string): Promise<User[]> {
  ...
  if (!isSupabaseConfigured) {
    ...
    return allUsers.filter(u => matchesUserQuery(u, searchQuery));
  }
  try {
    const escapedQuery = searchQuery.replace(/\\/g, '\\\\').replace(/"/g, '\\"');
    
    const orQueryStr = `full_name.ilike.%${escapedQuery}%,username.ilike.%${escapedQuery}%,mobileNumber.ilike.%${escapedQuery}%,contactNumber.ilike.%${escapedQuery}%`;
    console.log("[TRACE] 2. dbSearchUsers called with query:", searchQuery);
    console.log("[TRACE] 3. Exact Supabase .or() query:", orQueryStr);
    ...
    console.log("[TRACE] 4. Returned data:", data);
    console.log("[TRACE] 5. Returned data length:", data?.length);
...
// =========================================
// SHARE SYSTEM HELPERS
// =========================================
export async function dbCreateSharePost(data: { userId: string, entityType: string, entityId: string, caption: string, privacy: string }) {
  if (!supabase || !isSupabaseConfigured) return false;
  ...
  try {
    const { error } = await supabase.from('posts').insert({
      user_id: data.userId,
      post_type: 'general',
      text_content: '[SHARE_DATA]' + JSON.stringify({ caption: data.caption, entityType: data.entityType, entityId: data.entityId }),
    });
    if (error) throw error;

    await dbIncrementShareCount(data.entityType, data.entityId);
    return true;
  } catch (err) {
    console.error("dbCreateSharePost error", err);
    throw err;
  }
}

export async function dbCreateShareStory(data: { userId: string, entityType: string, entityId: string, caption: string, author: string, avatar: string }) {
  if (!supabase || !isSupabaseConfigured) return false;
  try {
    const { error } = await supabase.from('stories').insert({
      ...
      text: '[SHARE_DATA]' + JSON.stringify({ caption: data.caption, entityType: data.entityType, entityId: data.entityId }),
      type: 'share',
      createdAt: Date.now(),
      created_at: new Date().toISOString(),
    ...
    console.error("dbCreateShareStory error", err);
    throw err;
  }
}

async function dbIncrementShareCount(entityType: string, entityId: string) {
  if (!supabase || !isSupabaseConfigured) return;
  try {
    const tableName = getTableNameFromEntityType(entityType);
    if (!tableName) return;
    
    const { data: current } = await supabase.from(tableName).select('shares').eq('id', entityId).single();
    if (current) {
      await supabase.from(tableName).update({ shares: (current.shares || 0) + 1 }).eq('id', entityId);
    }
  } catch(e) {
    console.warn("Could not increment shares", e);
  }
}

function getTableNameFromEntityType(type: string) {
  switch(type) {
    case 'post': return 'posts';
    case 'job': return 'jobs';
    case 'property': return 'properties';
    case 'marketplace': return 'marketplace_items';
