...
async function testSearch(searchQuery) {
  ...
  // Test full_name
  console.log('\n--- Testing full_name ---');
  let res = await supabase.from('profiles').select('*').ilike('full_name', `%${escapedQuery}%`);
  console.log('Error:', res.error?.message);
  console.log('Data length:', res.data?.length);

  // Test username
  console.log('\n--- Testing username ---');
  res = await supabase.from('profiles').select('*').ilike('username', `%${escapedQuery}%`);
  console.log('Error:', res.error?.message);
  console.log('Data length:', res.data?.length);

  // Test mobileNumber
  console.log('\n--- Testing mobileNumber ---');
  res = await supabase.from('profiles').select('*').ilike('mobileNumber', `%${escapedQuery}%`);
  console.log('Error:', res.error?.message);
  console.log('Data length:', res.data?.length);
  
  // Test contactNumber
  console.log('\n--- Testing contactNumber ---');
  res = await supabase.from('profiles').select('*').ilike('contactNumber', `%${escapedQuery}%`);
  console.log('Error:', res.error?.message);
  console.log('Data length:', res.data?.length);
...
testSearch('test').then(() => console.log('Done'));
