...
export const STATIC_COUNTRIES: Country[] = [
  { id: 'country-pk-1', name: 'Pakistan', code: 'PK' }
];

export const STATIC_PROVINCES: Province[] = [
  { id: 'prov-punjab-1', name: 'Punjab', countryId: 'country-pk-1' }
];

export const STATIC_CITIES: City[] = [
  { id: 'city-rwp-1', name: 'Rawalpindi', provinceId: 'prov-punjab-1' }
];
...
export async function dbGetCountries(): Promise<Country[]> {
  if (!isSupabaseConfigured || !supabase) return STATIC_COUNTRIES;
  try {
    ...
    return data && data.length > 0 ? data : STATIC_COUNTRIES;
  } catch (err) {
    console.warn("dbGetCountries failed, using local fallback databases:", err);
    return STATIC_COUNTRIES;
  }
}

export async function dbGetProvinces(countryId: string): Promise<Province[]> {
  if (!isSupabaseConfigured || !supabase) {
    return STATIC_PROVINCES.filter(p => p.countryId === countryId);
  }
  try {
    const { data, error } = await supabase
      .from('provinces')
    ...
    })) : STATIC_PROVINCES.filter(p => p.countryId === countryId);
  } catch (err) {
...
export async function dbGetCities(provinceId: string): Promise<City[]> {
  if (!isSupabaseConfigured || !supabase) {
    return STATIC_CITIES.filter(c => c.provinceId === provinceId);
  }
  try {
    const { data, error } = await supabase
      .from('cities')
    ...
    })) : STATIC_CITIES.filter(c => c.provinceId === provinceId);
  } catch (err) {
