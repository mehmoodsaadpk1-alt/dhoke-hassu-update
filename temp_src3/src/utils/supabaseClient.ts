...
export async function dbGetStories(currentUserId: string, fallback: Story[] = []): Promise<Story[]> {
  return safeDbCall(async () => {
    ...
    const { data, error } = await supabase!
      ...
      .select(`
        *,
        author_profile:profiles!stories_userId_fkey(full_name, profile_photo)
      `)
...
export async function dbSaveStory(story: Story): Promise<boolean> {
  const payload = {
    ...
    custom_audience_ids: story.customAudienceIds,
...
export async function dbReactToStory(storyId: string, reactorId: string, reactionType: string): Promise<boolean> {
  try {
    ...
    // Notification
    const { data: story } = await supabase!.from('stories').select('user_id').eq('id', storyId).single();
    if (story && story.user_id !== reactorId) {
      ...
      if (reactor) {
        await dbTriggerNotification(
          ...
          'story_reaction',
          'New Story Reaction',
          `${reactor.full_name} reacted ${reactionType} to your story`,
          'story',
          storyId
        );
      }
    ...
    console.log("[STORY]", "Reaction Added:", reactionType);
    return true;
  } catch (e) {
    console.error("[STORY ERROR]", "Failed to add reaction", e);
    return false;
  }
}
...
export async function dbReplyToStory(storyId: string, senderId: string, replyType: string, content: string): Promise<boolean> {
  try {
    ...
    // Notification
    const { data: story } = await supabase!.from('stories').select('user_id').eq('id', storyId).single();
    if (story && story.user_id !== senderId) {
      ...
      if (sender) {
        await dbTriggerNotification(
          ...
          'story_reply',
          'New Story Reply',
          `${sender.full_name} replied to your story: "${content.substring(0, 50)}"`,
          'story',
          storyId
        );
      }
    ...
    console.error("[STORY ERROR]", "Failed to send reply", e);
    return false;
  }
}
...
export async function dbUploadStoryMedia(file: File): Promise<string | null> {
  ...
  try {
    ...
    if (error) {
      console.error("[STORY] Storage Upload Error details:", { message: error.message, name: error.name });
      return null;
    }
    ...
    console.error("[STORY] Exception in dbUploadStoryMedia:", { message: err.message, code: err.code });
    return null;
  }
}
...
export async function dbSavePoll(poll: Poll): Promise<boolean> {
  ...
  try {
    ...
    if (schema.columns.includes('title')) {
      ...
      pollPayload.question = poll.title; // Map title to question if legacy schema
    }
...
export async function dbSavePromotion(promo: any): Promise<boolean> {
  return safeWrite('promotions', promo);
}
...
export async function dbSearchUsers(searchQuery: string): Promise<User[]> {
  ...
  try {
    // Use ilike for case‑insensitive partial matching on both columns
    const { data, error } = await supabase
      .from('profiles')
      .select('*')
