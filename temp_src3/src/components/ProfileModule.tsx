...
import { Country, Province, City, Area } from '../types';
import {
  User as UserIcon,
  ShieldCheck,
  Calendar,
...
export default function ProfileModule({
  ...
  useEffect(() => {
    if (user) {
      const hasDifference = 
        ...
        user.area !== profileData.area ||
        user.mobileNumber !== profileData.mobileNumber ||
        user.verified !== profileData.verified ||
        user.reputationScore !== profileData.reputationScore;
  ...
  const handleApplyCrop = () => {
    ...
    if (ctx) {
      ...
      img.onload = () => {
        ...
        // Propagate changes based on current subview
        if (activeSubView === 'edit') {
          setEditAvatar(croppedUrl);
        } else {
          setProfileData(prev => ({
            ...prev,
            profilePhoto: croppedUrl
          }));
        }
  ...
  const [editCountryId, setEditCountryId] = useState(profileData.countryId || '');
  const [editProvinceId, setEditProvinceId] = useState(profileData.provinceId || '');
  const [editCityId, setEditCityId] = useState(profileData.cityId || '');
  const [editAreaId, setEditAreaId] = useState(profileData.areaId || '');
  const [editLat, setEditLat] = useState<number | undefined>(profileData.latitude);
  const [editLng, setEditLng] = useState<number | undefined>(profileData.longitude);
  ...
  useEffect(() => {
    async function loadCountries() {
      const list = await dbGetCountries();
  ...
  useEffect(() => {
    if (!editCountryId) return;
    async function loadProvinces() {
      const list = await dbGetProvinces(editCountryId);
  ...
  useEffect(() => {
    ...
    async function loadCities() {
      const list = await dbGetCities(editProvinceId);
      setCitiesList(list);
    }
    loadCities();
  }, [editProvinceId]);
  ...
  useEffect(() => {
    ...
    async function loadAreas() {
      const list = await dbGetAreas(editCityId);
      setAreasList(list);
    }
    loadAreas();
  }, [editCityId]);
  ...
  useEffect(() => {
    ...
    setEditCountryId(profileData.countryId || '');
  ...
  const renderAvatarModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto" id="avatar-crop-modal">
                ...
                <div className="w-full space-y-4">
                  ...
                  <div className="flex gap-3">
                    ...
                    <button
                      ...
                      className="flex-1 py-2.5 px-4 bg-[#2563eb] hover:bg-blue-600 text-white text-xs font-bold rounded-xl shadow-md transition-all cursor-pointer"
                    >
        ...
        <form onSubmit={handleSaveProfile} className="bg-white rounded-3xl border border-slate-200/70 shadow-sm overflow-hidden" id="edit-profile-form">
          ...
          <div className="pt-16 p-6 sm:p-8 space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">
                  {currentLanguage === 'en' ? 'Country' : 'ملک'} *
                </label>
                <select
                  value={editCountryId}
                  onChange={(e) => {
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-600 focus:bg-white transition-all font-semibold text-slate-800"
                >
                  <option value="" disabled>{currentLanguage === 'en' ? 'Select Country' : 'ملک منتخب کریں'}</option>
              ...
              <div>
                <label className="block text-xs font-bold text-slate-700 mb-1.5">
                  {currentLanguage === 'en' ? 'Province / State' : 'صوبہ / ریاست'} *
                </label>
                <select
                  ...
                  className="w-full px-4 py-2.5 bg-slate-50/50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-600 focus:bg-white transition-all font-semibold text-slate-800 disabled:opacity-50"
                >
              ...
              {/* GPS Auto-detection (Optional) */}
              <div className="sm:col-span-2">
                <div className="bg-slate-50/50 p-4 border border-slate-200/80 rounded-2xl flex flex-col items-center gap-2">
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      if (coords) {
                        setEditLat(coords.latitude);
                        setEditLng(coords.longitude);
                        if (editCityId) {
                          const nearest = await findNearestArea(coords.latitude, coords.longitude, editCityId);
                          if (nearest) {
                            setEditAreaId(nearest.id);
                          }
                        }
                      }
                      setLoadingGPS(false);
                  ...
                  {editLat !== undefined && editLng !== undefined && (
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 pb-12 animate-fade-in" id="profile-main-view-container">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/70 shadow-sm overflow-hidden relative">
        ...
        <div className="px-6 pb-6 relative flex flex-col md:flex-row items-center md:items-end justify-between gap-4 -mt-12 sm:-mt-16 z-10 text-center md:text-left">
          <div className="flex flex-col md:flex-row items-center md:items-end gap-4 sm:gap-6">
            {/* Avatar Frame */}
            <div className="w-28 h-28 sm:w-36 sm:h-36 rounded-full border-4 border-white bg-slate-100 overflow-hidden shadow-lg relative group shrink-0">
          ...
          {/* Edit Buttons */}
          <div className="flex items-center gap-2 md:mb-2 shrink-0 w-full md:w-auto justify-center">
            <button
              onClick={() => navigate('/profile/edit')}
              className="flex-1 md:flex-initial flex items-center justify-center gap-2 py-2 px-5 bg-blue-600 hover:bg-blue-700 text-white text-xs font-black rounded-xl shadow-md hover:shadow-lg transition-all cursor-pointer border-0"
