...
export default function AppShell({
      ...
      if (saved) {
        ...
        if (parsed.id === user.id || parsed.email === user.email) {
          return {
            ...
            joinDate: parsed.joinDate || undefined,
            reputationScore: parsed.reputationScore ?? authBase.reputationScore,
            verified: parsed.verified || authBase.verified || isEntityVerified(authBase.fullName) || false,
            coverPhoto: parsed.coverPhoto || undefined,
            contactNumber: parsed.contactNumber || undefined,
            socialLinks: parsed.socialLinks || undefined,
            badges: parsed.badges || undefined,
            profilePhoto: parsed.profilePhoto || authBase.profilePhoto,
            gender: parsed.gender || authBase.gender,
            dateOfBirth: parsed.dateOfBirth || authBase.dateOfBirth,
          };
        }
  ...
  React.useEffect(() => {
    ...
    setProfileData(prev => {
      const merged = {
        ...
        mobileNumber: prev.mobileNumber ? prev.mobileNumber : (user.mobileNumber || prev.mobileNumber),
        area: prev.area ? prev.area : (user.area || prev.area),
        profilePhoto: prev.profilePhoto ? prev.profilePhoto : (user.profilePhoto || prev.profilePhoto),
        gender: prev.gender ? prev.gender : (user.gender || prev.gender),
        dateOfBirth: prev.dateOfBirth ? prev.dateOfBirth : (user.dateOfBirth || prev.dateOfBirth),
        coverPhoto: prev.coverPhoto ? prev.coverPhoto : (user.coverPhoto || prev.coverPhoto),
        countryId: prev.countryId ? prev.countryId : (user.countryId || prev.countryId),
        provinceId: prev.provinceId ? prev.provinceId : (user.provinceId || prev.provinceId),
        cityId: prev.cityId ? prev.cityId : (user.cityId || prev.cityId),
        areaId: prev.areaId ? prev.areaId : (user.areaId || prev.areaId),
        latitude: prev.latitude !== undefined ? prev.latitude : (user.latitude || prev.latitude),
        longitude: prev.longitude !== undefined ? prev.longitude : (user.longitude || prev.longitude),
        socialLinks: {
          ...(prev.socialLinks || {}),
          ...(user.socialLinks || {})
        }
  ...
  }, [user.id, user.fullName, user.email, user.area, user.profilePhoto, user.gender, user.dateOfBirth, user.coverPhoto, user.countryId, user.provinceId, user.cityId, user.areaId, user.latitude, user.longitude]);
  ...
  // Persistent & seedable Stories state
  const [stories, setStories] = useState<Story[]>(() => {
    try {
      const stored = localStorage.getItem('dh_stories_list_v2');
      if (stored) {
    ...
    // Pre-seeded stories
    return mockStories.map((s, idx) => ({
      ...s,
      type: 'photo',
      createdAt: Date.now() - (idx + 1) * 3 * 60 * 60 * 1000 // 3h, 6h, 9h, 12h ago
    }));
  });

  React.useEffect(() => {
    try {
      localStorage.setItem('dh_stories_list_v2', JSON.stringify(stories));
    } catch (e) {
      console.warn("Storage quota exceeded, stories list could not be fully persisted to localStorage:", e);
      try {
        // Keep only the most recent 3 stories to free up space
        const trimmed = stories.slice(-3);
        localStorage.setItem('dh_stories_list_v2', JSON.stringify(trimmed));
      } catch (err) {
        localStorage.removeItem('dh_stories_list_v2');
      }
    }
  }, [stories]);
  ...
  const [posts, setPosts] = useState<Post[]>(() => {
    try {
      const stored = localStorage.getItem('dh_posts_list_v2');
      if (stored) {
        const parsed = JSON.parse(stored) as Post[];
        return parsed.map(cleanPostImagesMetadata);
      }
    } catch {}
    ...
    return mockPosts.map(post => {
      ...
      if (post.id === 'p1') {
        comments = [
          {
            ...
            content: 'Great news! Water supply was a big concern. Thank you for updating us.',
            time: '2 hours ago'
          },
          {
            id: 'c1_2',
            ...
            content: 'Thanks, Sajid! Water pressure is indeed back to normal in our lane too.',
            time: '1 hour ago'
          }
        ];
      } else if (post.id === 'p2') {
        comments = [
          {
            id: 'c2_1',
            author: 'Malik Shakeel',
            avatar: 'https://images.unsplash.com/photo-1535713875002-d1d0cf377fde?auto=format&fit=crop&q=80&w=120',
            content: 'Excellent initiative by Al-Khidmat Foundation. Highly appreciated!',
            time: '18 hours ago'
          }
        ];
      }
  ...
  React.useEffect(() => {
    try {
      localStorage.setItem('dh_posts_list_v2', JSON.stringify(posts));
    } catch (e) {
      console.warn("Storage quota exceeded, posts list could not be fully persisted to localStorage:", e);
      try {
        const trimmed = posts.slice(-5);
        localStorage.setItem('dh_posts_list_v2', JSON.stringify(trimmed));
      } catch (err) {
        localStorage.removeItem('dh_posts_list_v2');
      }
    }
  }, [posts]);
      ...
      try {
        ...
        if (profileData.id) {
          ...
          if (profile) {
            ...
            console.log("[AppShell Init] Profile not found. Clearing cached profile and logging out.");
            localStorage.removeItem('dh_user_profile_data');
            if (onLogout) {
              onLogout();
            }
    ...
    return (
      <div className="bg-white rounded-2xl p-4 shadow-sm border border-slate-200/60 mb-6" id="stories-bar-container">
        <div className="flex gap-4 overflow-x-auto pb-2 scroll-smooth no-scrollbar snap-x snap-mandatory">
          {/* Add Story Button */}
          <div 
            onClick={() => setIsStoryCamOpen(true)}
          ...
          {stories.map((story, idx) => (
            ...
            >
              <div className="relative w-14 h-14 rounded-full p-[2px] bg-gradient-to-tr from-blue-500 via-pink-500 to-amber-500 group-hover:scale-105 transition-transform duration-200">
                <img 
                  src={story.avatar} 
                  alt={story.author}
      ...
      <div className="bg-white rounded-md border border-slate-200/60 p-4 mb-6 shadow-xs space-y-4" id="premium-status-composer">
        ...
        <div className="flex flex-wrap items-center justify-between gap-3 pt-3 border-t border-slate-100">
          <div className="flex items-center gap-2">
            ...
            >
              <MapPin className={`w-4 h-4 ${composerLocation !== null ? 'text-blue-500' : 'text-slate-500'} ${isDetectingLocation ? 'animate-bounce' : ''}`} />
              <span>📍 {isDetectingLocation ? (currentLanguage === 'en' ? 'Locating...' : 'تلاش جاری...') : (currentLanguage === 'en' ? 'Location' : 'مقام')}</span>
            </button>
          </div>
        ...
        <div className="flex items-center gap-4 overflow-x-auto pb-2 pt-1 no-scrollbar scroll-smooth">
          ...
          <div className="flex flex-col items-center shrink-0">
            <button 
              ...
              className="relative w-15 h-15 rounded-full p-[3px] border-2 border-dashed border-slate-300 hover:border-blue-500 bg-slate-50 flex items-center justify-center transition-all cursor-pointer group"
              title="Create Status"
            >
                <img 
                ...
                />
                <div className="absolute bottom-0 right-0 bg-blue-500 text-white rounded-full w-5 h-5 flex items-center justify-center text-[10px] font-black border-2 border-white">+</div>
              </button>
              <span className="text-[10px] font-black text-slate-600 mt-2">
                {currentLanguage === 'en' ? 'Add Story' : 'کہانی'}
              </span>
            </div>

            {stories.map((story) => (
              <div className="flex flex-col items-center shrink-0" key={story.id}>
                <button
                  onClick={() => {
                    setActiveStory(story);
                    setIsStoryPaused(false);
                  }}
                  className="relative w-15 h-15 rounded-full p-[3px] bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 hover:scale-105 transition-all duration-200 cursor-pointer shadow-sm active:scale-95"
                >
                  <div className="w-full h-full rounded-full border-2 border-white overflow-hidden bg-slate-100">
                    {story.type === 'text' ? (
                      <div className={`w-full h-full bg-gradient-to-br ${story.bgColor || 'from-indigo-500 to-purple-500'} flex items-center justify-center p-1`}>
  ...
  const renderAddStoryModal = () => {
    ...
    return (
      <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto" id="add-story-modal">
        ...
        <div className="relative bg-slate-900 w-full sm:max-w-md aspect-[9/16] sm:rounded-3xl overflow-hidden shadow-2xl flex flex-col justify-between z-10">
          ...
          <div className="absolute top-0 left-0 right-0 p-4 bg-gradient-to-b from-black/80 to-transparent z-20 space-y-3">
            ...
            <div className="flex gap-1.5 w-full">
              {stories.map((_, sIdx) => {
                let fill = 0;
                if (sIdx < viewingStoryIdx) fill = 100;
                else if (sIdx === viewingStoryIdx) fill = storyTimer;
          ...
          <div className="absolute inset-y-1/2 left-4 right-4 flex justify-between z-20 pointer-events-none">
            ...
            <button
              disabled={viewingStoryIdx === stories.length - 1}
              onClick={(e) => {
                e.stopPropagation();
                setViewingStoryIdx(viewingStoryIdx + 1);
                setStoryTimer(0);
  ...
  const renderShareModal = () => {
    ...
    const handleConfirmShareChat = (targetName: string) => {
      const messageBody = `[Shared Community Post by ${sharingPost.author}]: "${sharingPost.content.slice(0, 50)}..."`;
      try {
      ...
      alert(currentLanguage === 'en' 
        ? `Post shared successfully to ${targetName}!` 
        : `پوسٹ کامیابی کے ساتھ ${targetName} کے چیٹ پر شیئر کر دی گئی ہے!`
      );
      setSharingPost(null);
    };

    const handleCopyLink = () => {
      const simulatedLink = `https://dh-connect.pk/feed/post/${sharingPost.id}`;
      navigator.clipboard.writeText(simulatedLink);
      alert(currentLanguage === 'en' ? 'Link copied to clipboard!' : 'لنک کاپی کریں!');
      setSharingPost(null);
    };

    return (
      <div className="fixed inset-0 bg-slate-900/80 backdrop-blur-md z-50 flex items-center justify-center p-4" id="share-post-modal">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl max-w-sm w-full overflow-hidden">
          <div className="p-5 border-b border-slate-100 flex items-center justify-between">
            <h3 className="font-black text-slate-900 text-sm">
              {currentLanguage === 'en' ? 'Share Post' : 'پوسٹ شیئر کریں'}
            </h3>
            <button 
          ...
          <div className="p-5 space-y-4">
            <button
              ...
              className="w-full py-3 px-4 bg-blue-50 hover:bg-blue-100 text-blue-700 text-xs font-bold rounded-xl transition-all flex items-center justify-center gap-2 cursor-pointer border border-blue-200"
            >
              🔗 {currentLanguage === 'en' ? 'Copy Simulated Link' : 'لنک کاپی کریں'}
            </button>

            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-wider text-slate-400 block px-1">
                {currentLanguage === 'en' ? 'Send to Neighborhood Chat' : 'ہمسایہ چیٹ پر بھیجیں'}
              </label>
              <div className="space-y-1.5 max-h-48 overflow-y-auto">
                {shareTargets.map(target => (
                  <button
                    key={target.id}
                    onClick={() => handleConfirmShareChat(target.name)}
                  ...
                  >
                    ...
                    <span className="text-xs font-bold text-slate-700 truncate flex-1">{target.name}</span>
                    <span className="text-[10px] bg-slate-100 px-2 py-0.5 rounded-md font-bold text-slate-500">Send</span>
                  </button>
                ))}
              </div>
            </div>
                ...
                {(() => {
                  ...
                  for (let i = 0; i < feedItems.length; i++) {
                    ...
                    if (item.feedType === 'post') {
                      ...
                      const count = postLikes[post.id] ?? post.likes;
                      ...
                      elements.push(
                        <div 
                          ...
                          className={`bg-white rounded-2xl border p-5 shadow-sm space-y-4 hover:shadow-md transition-all duration-200 cursor-pointer ${
                            isCritical 
                              ? 'border-2 border-red-500 shadow-red-50/50' 
                              : 'border-slate-200/60 hover:border-[#2563eb]/60'
                          }`}
                        >
                              ...
                              <div>
                                <div className="flex items-center gap-1.5">
                                  <h4 className="font-extrabold text-slate-900 text-sm">
                                    {currentLanguage === 'en' ? 'Critical Alert' : 'اہم علاقائی الرٹ'}
                                  </h4>
                                  <span className="text-[10px] text-slate-400">•</span>
                                  <span className={`px-2 py-0.5 rounded-full text-[8px] font-black uppercase tracking-wider border flex items-center gap-1 ${severityBg}`}>
                                    <span className={`w-1.5 h-1.5 rounded-full ${severityColor} ${isCritical ? 'animate-ping' : 'animate-pulse'}`} />
                                    {currentLanguage === 'en' 
                                      ? (alertItem.priority || alertItem.severity) 
                                      : (isCritical ? 'انتہائی اہم' : isHigh ? 'اہم' : 'معمولی')}
                                  </span>
                                </div>
                                <p className="text-[10px] text-slate-400 font-semibold">
                                  Posted by {alertItem.postedBy}
                                </p>
                              </div>
                            </div>
                          ...
                          {alertItem.image && (
                            <div className="rounded-xl overflow-hidden bg-slate-50 border border-slate-100">
                              <img src={alertItem.image} alt={alertItem.title} className="w-full h-auto object-contain block" />
                            </div>
                          )}

                          <div className="flex items-center justify-between pt-3 border-t border-slate-100 text-[10px] font-bold text-slate-400 font-mono">
                            <span className="flex items-center gap-1 text-slate-600 font-sans">
                              📍 {alertItem.area}
                            </span>
                            <span>
                              ⏱️ {alertItem.postedTime}
                            </span>
                          </div>
                        </div>
                      );
                      ...
                      elements.push(
                        <div key={poll.id} className={`bg-white rounded-2xl border p-5 shadow-sm space-y-4 border-slate-200/60 ${poll.featured ? 'ring-2 ring-indigo-500/20 bg-indigo-50/5' : ''}`}>
                          <div className="flex justify-between items-center">
                            <div className="flex items-center gap-3">
                              <div className="w-9 h-9 rounded-full bg-indigo-50 border border-indigo-100 flex items-center justify-center text-sm shadow-inner">
                                📊
                              </div>
                              <div>
                                <div className="flex items-center gap-1.5">
                                  <h4 className="font-extrabold text-slate-900 text-sm">
                                    {currentLanguage === 'en' ? 'Community Opinion Poll' : 'سروے اور رائے عامہ'}
                                  </h4>
                                  <span className="text-[10px] text-slate-400">•</span>
                                ...
                                <p className="text-[10px] text-slate-400 font-semibold">
                                  {currentLanguage === 'en' ? 'Dhoke Hassu Connect Portal' : 'ڈہوک حسو کنیکٹ پورٹل'}
                                </p>
                              </div>
                            </div>
                          ...
                          {poll.cover_image && (
                            <div className="rounded-xl overflow-hidden bg-slate-50 border border-slate-100">
                              <img src={poll.cover_image} alt={poll.title} className="w-full h-auto object-contain block" />
                            </div>
                          )}
                          ...
                          <div className="space-y-2.5 pt-1">
                            {hasVoted || isClosed ? (
                              // Render Percentage progress bars
