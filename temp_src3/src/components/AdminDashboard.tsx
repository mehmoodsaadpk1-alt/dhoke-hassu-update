...
export default function AdminDashboard({ currentLanguage, onExitAdmin }: AdminDashboardProps) {
  ...
  const handleAdminLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (adminPassword === 'admin123' || adminPassword === 'connect_admin') {
      ...
      setLoginError(isEn ? 'Invalid passcode credentials. Try "admin123"' : 'غلط پاس ورڈ۔ دوبارہ کوشش کریں۔');
    }
  };
  ...
  if (confirm(`Are you sure you want to delete user profile for ${displayName}? This is irreversible.`)) {
    ...
    if (isSupabaseConfigured && userId) {
      ...
      try {
        console.log("[DELETE USER] Attempting auth deletion for user ID:", userId);
        const { error: authError } = await supabase.auth.admin.deleteUser(userId);
        if (authError) {
          console.warn("[DELETE USER] Auth deletion error:", authError);
        } else {
          console.log("[DELETE USER] Auth user deleted successfully.");
        }
      } catch (authEx) {
        console.warn("[DELETE USER] Exception during auth deletion:", authEx);
      }
...
CREATE POLICY "Allow anyone to manage ads" ON public.ads FOR ALL USING (true) WITH CHECK (true);`}
              ...
              {[
                { path: '/admin/posts', label: isEn ? 'Community Feed Management' : 'کمیونٹی پوسٹس', moduleKey: 'posts', items: posts, searchKeys: ['author', 'content'] },
