...
import { dbGetAllStoryAds, dbSaveStoryAd, dbDeleteStoryAd } from '../utils/supabaseClient';
import { Plus, Edit2, Trash2, Pause, Play, BarChart2, Eye, MousePointerClick, FastForward, XCircle } from 'lucide-react';
...
export default function AdminStoryAds({ currentLanguage }: AdminStoryAdsProps) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!mediaUrl || !ctaLink) {
      alert("Media URL and CTA Link are required");
      return;
    }
            ...
            <div className="p-6 overflow-y-auto">
              <form id="story-ad-form" onSubmit={handleSave} className="space-y-4">
                ...
                <div>
                  ...
                  <input type="text" required value={mediaUrl} onChange={e => setMediaUrl(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="https://..." />
                </div>

                <div>
                  <label className="block text-xs font-bold text-slate-700 mb-1">CTA Link</label>
                  <input type="url" required value={ctaLink} onChange={e => setCtaLink(e.target.value)} className="w-full bg-slate-50 border border-slate-200 rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-500" placeholder="https://yourbusiness.com" />
                </div>
