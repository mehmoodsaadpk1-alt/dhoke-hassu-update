...
import { Country, Province, City, Area, User, Language } from '../types';
import { 
  dbGetCountries, 
  dbGetProvinces, 
  dbGetCities, 
...
export default function LocationSetupWizard({
  ...
  useEffect(() => {
    async function initLocations() {
      const countryList = await dbGetCountries();
      setCountries(countryList);
      
      // Auto-select Pakistan if available
      const pk = countryList.find(c => c.name.toLowerCase() === 'pakistan');
      if (pk) {
        setSelectedCountry(pk.id);
  ...
  useEffect(() => {
    ...
    async function loadProvinces() {
      const provList = await dbGetProvinces(selectedCountry);
      setProvinces(provList);
      
      const punjab = provList.find(p => p.name.toLowerCase() === 'punjab');
      if (punjab) {
        setSelectedProvince(punjab.id);
      } else if (provList.length > 0) {
        setSelectedProvince(provList[0].id);
      }
    }
    loadProvinces();
  }, [selectedCountry]);
  ...
  useEffect(() => {
    ...
    async function loadCities() {
      const cityList = await dbGetCities(selectedProvince);
      setCities(cityList);
  ...
  useEffect(() => {
    ...
    async function loadAreas() {
      ...
      if (areaList.length > 0) {
        // Default to Dhoke Hassu if matching
        const dh = areaList.find(a => a.name.toLowerCase() === 'dhoke hassu');
        setSelectedArea(dh ? dh.id : areaList[0].id);
      }
    }
    loadAreas();
  }, [selectedCity]);
  ...
  const handleDetectGPS = async () => {
    ...
    if (coords) {
      ...
      // Auto match nearest area if city is selected
      if (selectedCity) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedCountry || !selectedProvince || !selectedCity || !selectedArea) {
          ...
          <div className="space-y-3.5">
            ...
            <div className="space-y-1">
              ...
              <select
                ...
                className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-sm focus:outline-none focus:border-blue-500 transition-all font-semibold"
              >
