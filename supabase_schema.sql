...
-- ==========================================
-- PHASE 2: VIDEO PLATFORM FOUNDATION
-- ==========================================
...
CREATE POLICY "Anyone can read video_likes" 
...
CREATE POLICY "Anyone can read video_comments" 
...
CREATE POLICY "Users can insert views" 
    ...
    WITH CHECK (true); -- Anyone can insert a view (even anonymous)
...
-- End Phase 2
C R E A T E   U N I Q U E   I N D E X   I F   N O T   E X I S T S   i d x _ v i d e o _ v i e w s _ u n i q u e _ u s e r _ v i d e o   O N   p u b l i c . v i d e o _ v i e w s ( u s e r _ i d ,   v i d e o _ i d )   W H E R E   u s e r _ i d   I S   N O T   N U L L ;  
 
-- ============================================================================
-- MILESTONE 7: VIDEO COMMENTS SCHEMA
-- ============================================================================
...
CREATE POLICY "Anyone can view reactions" ON public.video_comment_reactions
    FOR SELECT USING (true);

-- Authenticated users can insert/delete reactions
CREATE POLICY "Authenticated users can react" ON public.video_comment_reactions
    FOR ALL USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);


-- ============================================================================
-- MILESTONE 8: PERFORMANCE INDEXES FOR VIDEOS
-- ============================================================================
...
-- ============================================================================
-- AUDIT FIX MIGRATION (Phase 3)
-- ============================================================================

-- Fix Insecure RLS Policies

-- Profiles
DROP POLICY IF EXISTS "Allow anyone to insert or update profiles" ON public.profiles;
CREATE POLICY "Allow public read of profiles" ON public.profiles FOR SELECT USING (true);
CREATE POLICY "Allow authenticated users to insert own profile" ON public.profiles FOR INSERT WITH CHECK (auth.role() = 'authenticated' AND auth.uid() = user_id);
CREATE POLICY "Allow users to update own profile" ON public.profiles FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);

-- Polls
DROP POLICY IF EXISTS "Allow anyone to manage polls" ON public.polls;
CREATE POLICY "Allow authenticated users to manage own polls" ON public.polls FOR ALL USING (auth.uid() = created_by) WITH CHECK (auth.uid() = created_by);

-- Promotions
DROP POLICY IF EXISTS "Allow anyone to manage promotions" ON public.promotions;
CREATE POLICY "Allow authenticated users to manage own promotions" ON public.promotions FOR ALL USING (auth.uid() = created_by) WITH CHECK (auth.uid() = created_by);

-- Ads
DROP POLICY IF EXISTS "Allow anyone to manage ads" ON public.ads;
CREATE POLICY "Allow authenticated users to manage own ads" ON public.ads FOR ALL USING (auth.uid() = advertiser_id) WITH CHECK (auth.uid() = advertiser_id);
...
INSERT INTO storage.buckets (id, name, public)
VALUES ('dh_videos', 'dh_videos', true)
ON CONFLICT (id) DO NOTHING;

-- RLS for dh_videos bucket
-- Allow public read access to all files in dh_videos
CREATE POLICY "Allow public read access to dh_videos bucket" 
ON storage.objects FOR SELECT 
USING (bucket_id = 'dh_videos');

-- Allow authenticated users to insert into dh_videos bucket
CREATE POLICY "Allow authenticated inserts to dh_videos bucket" 
ON storage.objects FOR INSERT 
WITH CHECK (bucket_id = 'dh_videos' AND auth.role() = 'authenticated');

-- Allow authenticated users to delete their own uploads from dh_videos bucket
CREATE POLICY "Allow authenticated deletes from dh_videos bucket" 
ON storage.objects FOR DELETE 
USING (bucket_id = 'dh_videos' AND auth.role() = 'authenticated' AND owner = auth.uid());
