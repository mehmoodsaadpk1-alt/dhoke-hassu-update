...
var import_meta = {};
var supabaseUrl = import_meta.env?.VITE_SUPABASE_URL || import_meta.env?.NEXT_PUBLIC_SUPABASE_URL || "";
var supabaseAnonKey = import_meta.env?.VITE_SUPABASE_ANON_KEY || import_meta.env?.NEXT_PUBLIC_SUPABASE_ANON_KEY || "";
var isSupabaseConfigured = !!(supabaseUrl && supabaseAnonKey);
if (!isSupabaseConfigured) {
  console.warn(
    "\u26A0\uFE0F Supabase URL or Anon Key is missing. Dhoke Hassu Connect is operating in Local/Offline mode. To connect to your real Supabase backend, please add VITE_SUPABASE_URL/NEXT_PUBLIC_SUPABASE_URL and VITE_SUPABASE_ANON_KEY/NEXT_PUBLIC_SUPABASE_ANON_KEY to your environment."
  );
}
...
var PushNotificationService = class {
  ...
  static async sendPush(userId, title, body, data) {
    try {
      ...
      if (isSupabaseConfigured && supabase) {
        const { data: dbTokens } = await supabase.from("user_push_tokens").select("token, platform").eq("user_id", userId);
        if (dbTokens) {
          tokens = dbTokens.map((t) => ({ token: t.token, platform: t.platform, userId }));
        }
      }
      if (tokens.length === 0) {
      ...
      for (const t of tokens) {
        ...
        if (t.platform === "web" && "Notification" in window && Notification.permission === "granted") {
